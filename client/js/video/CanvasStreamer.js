...
class CanvasStreamer {
	...
	constructor(divToPlayIn, useWorker, webgl) {
		this.divToPlayIn = divToPlayIn;
		this.webgl = webgl;
	...
	resize() {
		//handle resizes here instead of 
	}
	start() {
		if (!window.player) {
			var playerElement = document.getElementById(this.divToPlayIn)
			window.player = new Player({ useWorker: this.useWorker, webgl: this.webgl, size: { width: playerElement.innerWidth, height: playerElement.innerHeight } })

			setInterval(() => {
					window.player.canvas.style.width = `${window.player.canvas.width}px`;
					window.player.canvas.style.height = `${window.player.canvas.height}px`;
			}, 10);
			...
			// })
			//	window.debugger = new debug(this.divToPlayIn) //show statistics, you can remove me if you dont need stats
		}
		...
		socketHandler.addBinaryMessageHandler(async function (msg) {
			msg.data.arrayBuffer().then(arrayBuffer => {
				// console.log("data??")
				let view = new Uint8Array(arrayBuffer)
				//[v,i,d,e,o]
				if (view[0] === 118 && view[1] === 105) {
					// console.log("videofound")
					// view.slice(5)
					// this.appendBuffer(0, view.slice(5))
					this.framesReceived++;
					this.bytesReceived += arrayBuffer.byteLength;
					window.player.decode(new Uint8Array(addSeparator(view.slice(5))))
				}
			})
		}.bind(this));
